...
def scan_directory(dir_config, cm_client, failed_archive_dir):
    ...
    file_pattern = dir_config.get("file_pattern", "*") # Default to all files
    recursive_scan = dir_config.get("recursive_scan", False)
    # Metadata to associate (example, can be expanded or made more dynamic)
    # For now, let's keep metadata simple, focusing on source path.
    # More complex metadata handling can be added later if needed from config.
    ...
    processed_files_in_current_scan = 0 # To track if any files were processed in this scan cycle for this dir_config
    ...
    if recursive_scan:
        for root, _, files in os.walk(path):
            for filename in files:
                if fnmatch.fnmatch(filename, file_pattern):
                    ...
                    # Check if cm_client is None (e.g. initial connection failed and wasn't recovered)
                    if not cm_client:
                        logging.error(f"CMClient not available. Cannot process file {file_path}.")
                        return # Exit scan for this directory if client is broken

                    logging.info(f"Found file: {filename} in {root}")
                    try:
                        # Pass item_type and basic metadata
                        upload_metadata = {"source_filename": filename, "original_path": file_path}
                        # doc_id = cm_client.upload_document(file_path, item_type, metadata={"source_path": file_path}) # Old
                        doc_id = cm_client.upload_document(file_path, item_type, metadata=upload_metadata)
                        
                        if doc_id:
                            processed_files_in_current_scan += 1
                            logging.info(f"Successfully uploaded {filename}, received DocID: {doc_id}")
                            if action_after_upload == "delete":
                                ...
                                if not move_target_dir:
                                    logging.warning(f"Move action specified for {path} but no move_target_directory configured.")
                                    continue
                                
                                # For recursive scans, try to preserve subdirectory structure in move_target_dir
                                relative_path = os.path.relpath(root, path)
                                current_move_target_dir = os.path.join(move_target_dir, relative_path)

                                if not os.path.exists(current_move_target_dir):
                                    try:
                                        os.makedirs(current_move_target_dir, exist_ok=True)
                                    except OSError as e:
                                        logging.error(f"Failed to create move target subdirectory {current_move_target_dir}: {e}")
                                        # Fallback: try to move to the root of move_target_dir or skip
                                        current_move_target_dir = move_target_dir # Attempt to move to root if sub-path fails
                                        if not os.path.exists(current_move_target_dir): # Check again for root
                                            logging.error(f"Root move target directory {current_move_target_dir} also not creatable/accessible. Skipping move for {file_path}")
                                            continue 
                                
                                try:
                                    shutil.move(file_path, os.path.join(current_move_target_dir, filename))
                                    logging.info(f"Moved file {file_path} to {os.path.join(current_move_target_dir, filename)}")
                                except (OSError, shutil.Error) as e:
                                    logging.error(f"Failed to move file {file_path} to {os.path.join(current_move_target_dir, filename)}: {e}")
                        else:
                            logging.warning(f"Failed to upload {filename} from {file_path}. Moving to failed archive.")
                            move_to_failed_archive(file_path, failed_archive_dir, reason_subdir="upload_failure")
                    except CMConnectionError as e:
                        logging.error(f"CM connection error during operation for {file_path}: {e}. Pausing daemon.")
                        global daemon_paused_due_to_cm_outage
                        daemon_paused_due_to_cm_outage = True
                        return # Exit this scan_directory call; main loop will handle pause.
                    except Exception as e:
                        logging.error(f"Unexpected error processing file {file_path}: {e}")
                        move_to_failed_archive(file_path, failed_archive_dir, reason_subdir="unexpected_error")
        ...
        for filename in os.listdir(path):
            ...
            if os.path.isfile(file_path) and fnmatch.fnmatch(filename, file_pattern):
                ...
                logging.info(f"Found file: {filename} in {path}")
                try:
                    upload_metadata = {"source_filename": filename, "original_path": file_path}
                    # doc_id = cm_client.upload_document(file_path, item_type, metadata={"source_path": file_path}) # Old
                    doc_id = cm_client.upload_document(file_path, item_type, metadata=upload_metadata)
                    ...
                    if doc_id:
                        processed_files_in_current_scan += 1
                        logging.info(f"Successfully uploaded {filename}, received DocID: {doc_id}")
                        # ... (Identical post-upload delete/move logic as in recursive block, but current_move_target_dir is just move_target_dir)
                        if action_after_upload == "delete":
                            try:
                                os.remove(file_path)
                                logging.info(f"Deleted file after upload: {file_path}")
                            ...
                            if not move_target_dir:
                                logging.warning(f"Move action specified for {path} but no move_target_directory configured.")
                                continue
                            if not os.path.exists(move_target_dir): # Ensure base move_target_dir exists
                                try:
                                    os.makedirs(move_target_dir, exist_ok=True)
                                except OSError as e:
                                    logging.error(f"Failed to create move target directory {move_target_dir}: {e}")
                                    continue
                            try:
                                shutil.move(file_path, os.path.join(move_target_dir, filename))
                                logging.info(f"Moved file {file_path} to {os.path.join(move_target_dir, filename)}")
                            except (OSError, shutil.Error) as e:
                                logging.error(f"Failed to move file {file_path} to {os.path.join(move_target_dir, filename)}: {e}")
                    else:
                        logging.warning(f"Failed to upload {filename} from {file_path}. Moving to failed archive.")
                        move_to_failed_archive(file_path, failed_archive_dir, reason_subdir="upload_failure")
                except CMConnectionError as e:
                    ...
                    global daemon_paused_due_to_cm_outage
                    daemon_paused_due_to_cm_outage = True
                    return # Exit this scan_directory call
                except Exception as e:
                    logging.error(f"Unexpected error processing file {file_path}: {e}")
                    move_to_failed_archive(file_path, failed_archive_dir, reason_subdir="unexpected_error")
    
    if processed_files_in_current_scan > 0:
        logging.info(f"Finished scanning {path}. Processed {processed_files_in_current_scan} files in this cycle.")
    else:
        logging.info(f"Finished scanning {path}. No new files matching criteria were processed in this cycle.")
...
def _process_single_file_upload(file_path, item_type, dir_config, cm_client, failed_archive_dir):
    # dir_config is passed to get action_after_upload, move_target_dir, path (for recursive move root), and recursive_scan flag
    ...
    try:
        ...
        if doc_id:
            ...
            if action_after_upload == "delete":
                ...
                # For recursive scans, preserve subdirectory structure relative to the initial scan path.
                if dir_config.get("recursive_scan", False) and scan_root_path != current_file_root:
                    ...
                    if relative_path and relative_path != '.': # Avoid joining if current_file_root is same as scan_root_path
                ...
                if not os.path.exists(effective_move_target_dir):
                    try:
                        ...
                        logging.error(f"Failed to create move target directory {effective_move_target_dir}: {e}. Attempting to move to root move_target_dir.")
                        effective_move_target_dir = move_target_dir # Fallback
                        if not os.path.exists(effective_move_target_dir): # Try creating root if it also doesn't exist
                             try: os.makedirs(effective_move_target_dir, exist_ok=True)
        ...
        else: # doc_id is None (and no CMConnectionError was raised by cm_client.upload_document)
        ...
        # Re-raise CMConnectionError to be caught by the main thread iterating futures.
        # This signals that this specific failure is due to a connection issue.
        raise 
    except OSError as e: # For file operations like os.remove/shutil.move if they fail unexpectedly at top level
    ...
    except Exception as e: # Catch any other unexpected error from CMClient or local ops
        logging.error(f"Unexpected error processing file {file_path} in thread: {e}", exc_info=True)
        if os.path.exists(file_path): # Ensure file exists before attempting to move
...
def scan_directory(dir_config, cm_client, failed_archive_dir, global_config): # Added global_config parameter
    global daemon_paused_due_to_cm_outage # Make sure this is accessible
    ...
    max_uploads = global_config.get("performance", {}).get("max_parallel_uploads", 1) # Default to 1 if not configured
    if not isinstance(max_uploads, int) or max_uploads <= 0: # Ensure positive integer
        logging.warning(f"Invalid max_parallel_uploads value ({max_uploads}), defaulting to 1.")
        max_uploads = 1

    files_to_process = []
    # Populate files_to_process list
    if recursive_scan:
        for root, _, files in os.walk(path):
            if daemon_paused_due_to_cm_outage: 
    ...
    with ThreadPoolExecutor(max_workers=max_uploads) as executor:
        ...
        for fp in files_to_process:
            ...
            if not cm_client: # Should be caught earlier, but as a safeguard in loop
            ...
            # Pass dir_config as it contains action_after_upload, move_target_dir, etc.
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                daemon_paused_due_to_cm_outage = True
                # Attempt to cancel remaining futures. Python 3.9+ for executor.shutdown(cancel_futures=True)
                # For broader compatibility, we break the loop and rely on the main loop's pause logic.
                # Any already running tasks will complete or error out.
                # New tasks are not submitted due to the check in the submission loop above.
                if hasattr(executor, '_threads'): # Basic check if executor is still active
                    # For Python 3.9+ one could do:
                    # for f_to_cancel in futures: # Iterate over the original dict keys
                    #    if not f_to_cancel.done() and not f_to_cancel.cancelled():
                    #        f_to_cancel.cancel()
                    # More robustly, just ensure no new tasks are submitted and let existing ones finish/error.
                    pass
...
def scan_directory(dir_config, cm_client, failed_archive_dir, global_config): # Added global_config parameter
    ...
    if submitted_files_count > 0:
        logging.info(f"Finished scanning {path}. Submitted {submitted_files_count} files for processing, {successful_uploads_count} successful uploads confirmed in this cycle.")
    elif not daemon_paused_due_to_cm_outage : 
        logging.info(f"Finished scanning {path}. No files were submitted for processing in this cycle (either none found or daemon was already paused).")
            ...
            if action_after_upload == "delete":
                ...
                if not os.path.exists(effective_move_target_dir):
                    try:
                        ...
                        logging.error(f"Failed to create move target directory {effective_move_target_dir}: {e}. Attempting to move to root move_target_dir.")
                        effective_move_target_dir = move_target_dir # Fallback to root
                        if not os.path.exists(effective_move_target_dir): # Try creating root if it also doesn't exist
                             try: os.makedirs(effective_move_target_dir, exist_ok=True)
                             except OSError as e_root:
                                logging.error(f"Root move target directory {effective_move_target_dir} also not creatable: {e_root}. Skipping move for {file_path}")
...
# Helper function for scan_directory
def _process_single_file_upload(file_path, item_type, dir_config, cm_client, failed_archive_dir):
    # Note: global_config_snapshot removed as it's not used, direct config values preferred if needed.
    filename = os.path.basename(file_path)
    action_after_upload = dir_config.get("action_after_upload")
    move_target_dir = dir_config.get("move_target_directory")
    scan_root_path = dir_config.get("path") 
...
# The old scan_directory function is replaced by the new one above.
# The helper _process_single_file_upload is defined before the new scan_directory.
pass # Ensures this block is distinct and the tool can replace the old scan_directory
